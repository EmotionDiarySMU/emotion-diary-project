...
// [V40 수정] JFrame 임포트 제거
import javax.swing.JPanel; // 컴포넌트들을 담는 패널(컨테이너)
// [V40 수정] SwingUtilities, UIManager 임포트 제거 (AppLauncher로 이동)
 ...
 * (V40 수정) JFrame -> JPanel로 변경. 'main' 메소드 제거.
 */
public class MainApplication extends JPanel { // [V40 수정] JFrame -> JPanel
    ...
    /**
     * MainApplication 생성자
     * (V40 수정) 이 객체가 생성될 때(new MainApplication()),
     * 'JPanel'로서의 GUI 컴포넌트를 즉시 초기화합니다.
     */
    public MainApplication() {
        // 1. [V40 수정] JPanel의 기본 속성 설정
        
        // [V40 제거] setTitle("Emotion Diary ☺️");
        // [V40 제거] setSize(550, 750); 
        // [V40 제거] setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        // [V40 제거] setLocationRelativeTo(null); 
        
        // 이 패널(JPanel)의 전체 레이아웃을 BorderLayout (동/서/남/북/중앙)으로 설정합니다.
        setLayout(new BorderLayout());

        // (디자인) 메인 패널의 기본 배경색을 파스텔 블루로 설정합니다.
        // [V40 수정] getContentPane().setBackground(PASTEL_BLUE); -> setBackground(PASTEL_BLUE);
        ...
        // (디자인) mainCardPanel을 투명하게 설정하여, 부모(MainApplication)의 파스텔 블루 배경색이 보이도록 합니다.
        ...
        // 5. 메인 패널(JPanel)에 네비게이션 패널과 메인 패널을 배치
        
        // 상단 네비게이션 패널을 이 패널(MainApplication)의 "NORTH"(북쪽) 영역에 추가합니다.
        add(navPanel, BorderLayout.NORTH);
        // 중앙 메인 패널을 이 패널(MainApplication)의 "CENTER"(중앙) 영역에 추가합니다.
    ...
    // [V40 제거] public static void main(String[] args) { ... }
}