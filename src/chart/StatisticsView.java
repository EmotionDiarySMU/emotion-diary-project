...
// [V38] '현실적인' 날짜 계산을 위해 'java.time' 임포트
import java.time.LocalDate;
import java.time.DayOfWeek;
import java.time.temporal.WeekFields;
import java.util.Locale;
import java.util.ArrayList; // 콤보박스 모델을 '동적'으로 생성하기 위해 임포트
import java.util.List;
import java.time.YearMonth; // [V39 버그 수정] 'YearMonth' 임포트 누락 수정
 ...
 * (V38 수정) 날짜 콤보박스(YEARS, MONTHS, WEEKS)가 '오늘 날짜' 기준으로 '동적'으로 생성되도록 변경되었습니다.
 */
public class StatisticsView extends JPanel { 
    ...
    // [V38 수정] '오늘 날짜' 정보를 멤버 변수로 저장 (동적 콤보박스 생성에 필요)
    ...
    private void initUI() {
        ...
        // [V38 수정] '오늘 날짜' 정보를 멤버 변수에 저장합니다.
        today = LocalDate.now(); // 예: 2025-11-07
        currentYear = today.getYear(); // 예: 2025
        currentMonth = today.getMonthValue(); // 예: 11
        // (정책) '주차'는 '월요일' 기준으로 계산합니다.
        WeekFields weekFields = WeekFields.of(DayOfWeek.MONDAY, 1);
        currentWeek = today.get(weekFields.weekOfMonth()); // 예: 11/7일은 '2주차'
        ...
        // --- [V40 버그 수정] 'initUI'의 초기화 순서를 (생성 -> 선택 -> 갱신)으로 수정 ---
        
        // 1. '연도' 콤보박스 모델 생성 (2020 ~ '올해')
        populateYearCombos(); 
        
        // 2. '연도' 콤보박스의 "기본 선택값"을 '올해'(2025년)로 "먼저" 설정
        yearComboW.setSelectedItem(currentYear + "년");
        yearComboM.setSelectedItem(currentYear + "년");
        yearComboY.setSelectedItem(currentYear + "년");

        // 3. '월' 콤보박스 갱신 (이제 'year'가 "2025년"이므로, 'maxMonth'가 11월로 정상 계산됨)
        updateMonthCombos(); 

        // 4. '월' 콤보박스의 "기본 선택값"을 '이달'(11월)로 "먼저" 설정
        monthComboW.setSelectedItem(String.format("%02d월", currentMonth));
        monthComboM.setSelectedItem(String.format("%02d월", currentMonth));

        // 5. '주' 콤보박스 갱신 (이제 'month'가 "11월"이므로, 'maxWeek'가 2주로 정상 계산됨)
        updateWeekCombo(); 

        // 6. '주' 콤보박스의 "기본 선택값"을 '이번 주'(2주)로 설정
        weekComboW.setSelectedItem(currentWeek + "주");
        
        // --- [V40 버그 수정 끝] ---


        // --- [V38 수정] 콤보박스 간 '연동'을 위한 리스너 추가 ---
        
        // "주간" 탭의 '연도' 콤보박스를 변경했을 때
        yearComboW.addActionListener(e -> updateMonthCombos()); // '월' 콤보박스를 갱신
        
        // "주간" 탭의 '월' 콤보박스를 변경했을 때
        monthComboW.addActionListener(e -> updateWeekCombo()); // '주' 콤보박스를 갱신
        
        // "월간" 탭의 '연도' 콤보박스를 변경했을 때
        yearComboM.addActionListener(e -> updateMonthCombos()); // '월' 콤보박스를 갱신
        ...
        weeklyCard.add(yearComboW); // "주간" 패널에 (동적) 년 콤보박스 추가
        weeklyCard.add(monthComboW); // "주간" 패널에 (동적) 월 콤보박스 추가
        weeklyCard.add(weekComboW); // "주간" 패널에 (동적) 주 콤보박스 추가
        ...
        monthlyCard.add(yearComboM); // "월간" 패널에 (동적) 년 콤보박스 추가
        monthlyCard.add(monthComboM); // "월간" 패널에 (동적) 월 콤보박스 추가
        ...
        yearlyCard.add(yearComboY); // "연간" 패널에 (동적) 년 콤보박스 추가
        ...
        // (참고: 이 리스너는 V35+부터 'Controller'가 오버라이드(override)하므로,
        //        '가짜 데이터'를 갱신하는 이 코드는 Controller 연결 시 무시됩니다.)
    ...
    // --- [V38 신규] 동적 콤보박스 생성 및 갱신 헬퍼 메소드 ---

    /**
     * (V38) '연도' 콤보박스들(W, M, Y)을 '오늘' 기준으로 (2020 ~ '올해') 채웁니다.
     */
    private void populateYearCombos() {
        List<String> years = new ArrayList<>();
        // 2020년부터 '올해'(currentYear)까지 반복
        ...
        // 3개의 '연도' 콤보박스 모델을 '동적'으로 생성된 'yearArray'로 설정
    ...
    /**
     * (V38) '월' 콤보박스들(W, M)을 '선택된 연도' 기준으로 (1월 ~ '이번 달') 채웁니다.
     * ('연도' 콤보박스가 변경될 때마다 호출됩니다.)
     */
    private void updateMonthCombos() {
        // 현재 '주간' 탭과 '월간' 탭에서 선택된 '연도'를 가져옵니다.
        ...
        // '주간' 탭의 '월' 콤보박스 갱신
        // 선택된 연도가 '올해'(2025년)이면, '이번 달'(11월)까지만 채웁니다.
        int maxMonthW = (yearW == currentYear) ? currentMonth : 12;
        monthComboW.setModel(createMonthModel(maxMonthW));
        // (갱신 후) 콤보박스의 기본 선택값이 '이번 달'을 넘지 않도록 조정
        if (yearW == currentYear && monthComboW.getSelectedIndex() == -1) {
            monthComboW.setSelectedItem(String.format("%02d월", currentMonth));
        } else if (monthComboW.getSelectedIndex() == -1) { // (V39 수정) 과거 연도 선택 시
            monthComboW.setSelectedIndex(0); // 1월을 기본 선택
        }

        // '월간' 탭의 '월' 콤보박스 갱신 (로직 동일)
        int maxMonthM = (yearM == currentYear) ? currentMonth : 12;
        monthComboM.setModel(createMonthModel(maxMonthM));
        if (yearM == currentYear && monthComboM.getSelectedIndex() == -1) {
            monthComboM.setSelectedItem(String.format("%02d월", currentMonth));
        } else if (monthComboM.getSelectedIndex() == -1) { // (V39 수정) 과거 연도 선택 시
            monthComboM.setSelectedIndex(0); // 1월을 기본 선택
        }
    }

    /**
     * (V38) '주' 콤보박스(W)를 '선택된 연도/월' 기준으로 (1주 ~ '이번 주') 채웁니다.
     * ('월' 콤보박스가 변경될 때마다 호출됩니다.)
     */
    private void updateWeekCombo() {
        ...
        if (year == currentYear && month == currentMonth) {
            maxWeek = currentWeek; // '이번 주'(2주)까지만 채웁니다.
        } else {
            // (현실성) 과거의 달(예: 2024년 2월)이 4주까지만 있는지 5주까지만 있는지 계산
            YearMonth ym = YearMonth.of(year, month); // [V39 버그 수정] 'YearMonth' 임포트 필요
        ...
        if (year == currentYear && month == currentMonth && weekComboW.getSelectedIndex() == -1) {
            weekComboW.setSelectedItem(currentWeek + "주");
        } else if (weekComboW.getSelectedIndex() == -1) { // (V39 수정) 과거 연/월 선택 시
             weekComboW.setSelectedIndex(0); // 1주를 기본 선택
        }
    }

    /**
     * (V38) 1월부터 'maxMonth'월까지의 '월' 콤보박스 모델을 생성하는 헬퍼 메소드
     */
    ...
    /**
     * (V38) 1주부터 'maxWeek'주까지의 '주' 콤보박스 모델을 생성하는 헬퍼 메소드
     */
