...
public class SearchDiaryPanel extends JPanel{
	...
	private static String currentOrder = "DESC"; // 기본값: 최신순
	...
	public SearchDiaryPanel() {
		...
		serchBt.addActionListener(e -> {
			...
			String startDate = firstDS.getSelectedDate();
			String endDate = secondDS.getSelectedDate();

			searchDiaryList(title, startDate, endDate);
		});
	...
	public static void refreshDiaryList() {
        try {
        	diaryEntries = DatabaseManager.getAllEntries(currentOrder);
	...
	public static void searchDiaryList(String title, String startDate, String endDate) {
        try {
        	System.out.println("=== 일기 검색 ===");
        	System.out.println("제목: '" + title + "'");
        	System.out.println("시작 날짜: " + startDate);
        	System.out.println("종료 날짜: " + endDate);
        	System.out.println("정렬 순서: " + currentOrder);

        	diaryEntries = DatabaseManager.searchEntries(title, startDate, endDate, currentOrder);

        	System.out.println("검색 결과: " + diaryEntries.size() + "개");

            diaryModel.clear(); // 기존 항목 제거

            if (diaryEntries.isEmpty()) {
            	// 검색 결과가 없을 때 메시지 표시
            	diaryModel.addElement("검색 결과가 없습니다.");
            } else {
	            for (DiaryEntry e : diaryEntries) {
	                diaryModel.addElement(e.getTitle() + " (" + e.getEntry_date() + ")");
	            }
